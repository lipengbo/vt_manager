...
function init(islandId) {
    ...
    $.get(url, function success (responseTxt, statusTxt, xhr) {
        if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
            ...
            fsTop.root_ovs = responseTxt.root_ovs || [];
            fsTop.links_root = responseTxt.links_root || [];
        } else {
...
var getRootId2 = function (visited) {  // ovs gre first
    "use strict";
    var maxId,
    	i;
	for(i in fsTop.root_ovs){
		if (visited.indexOf(fsTop.root_ovs[i]) === -1) {
			maxId = [fsTop.root_ovs[i]];
			break;
		}
	}
    return maxId;
...
function drawer (base, maxWidth, delta_y, treesWidth) {
    ...
    this.base = [base.left + this.width/2, base.top + delta_y*0.5];
    this.max_y = 200;
    this.baseAngle = Math.PI*2/3; 
...
//            };
        var that   = this,
        host_r = (level+0.6)* that.delta_y,
        ovs_r  = (level+1)* that.delta_y,
        x      = function (item, r) {
            return (function () {
                return that.base[0] - Math.sin(Math.PI/3 - (seq.indexOf(item) + 1) * that.baseAngle/(seq.length+1)) * r;
            })();
        },
        host_y = function (item) {
            return (function () {
                return that.base[1]+(level + 1 )*that.delta_y + host_r * Math.cos(Math.PI/3-(seq.indexOf(item) + 1) * that.baseAngle/(seq.length+1));
            })();
        },
        ovs_y = function (item) {
            return (function () {
                return that.base[1]+(level + 1 )*that.delta_y + ovs_r * Math.cos(Math.PI/3- seq.indexOf(item) * that.baseAngle/seq.length);
            })();
        };
        this.max_y = (this.max_y > this.base[1] + (level+1) * this.delta_y) ? this.max_y : this.base[1] + (level+1) * this.delta_y;
        
        if (level === -1) {
            seq.forEach(function (item){
                that['flowvisor'][item]= [x(item, ovs_r), ovs_y(item)];
            });
        } else {
            ovs.forEach(function (item){
                that['ovs'][item]= [x(item, ovs_r), ovs_y(item)];
                that.max_y = (that.max_y > ovs_y(item)) ? that.max_y : ovs_y(item);
            });
            minus(seq, ovs).forEach(function (item, i){
                that['host'][item]= [x(item, host_r), host_y(item)];
                that.max_y = (that.max_y > host_y(item)) ? that.max_y : host_y(item);
            });
        }
    },
            ...
            if (vertex.hasOwnProperty(nodeType)) {
                ...
                for (nodeId in vertex[nodeType]){
                    if (vertex[nodeType].hasOwnProperty(nodeId)) {
                        ...
                        x = coordinate[0] - vertexBox.pic_width * 0.5;
                        y = coordinate[1] - vertexBox.pic_height * 0.5;
                        html += ["<a xlink:href='/host/overview/", nodeId, "' target='new'>",
                            "<image xlink:href='" , PIC_PATH , nodeType , ".png' ", "x='", x, "' y='", y, "'",
                            "width='", vertexBox.pic_width, "'height='", vertexBox.pic_height, "'>", "</a>"].join(''); 
                    }
                }
            }
        }
...
function draw (obj, PIC_PATH) {
    ...
    obj.append(show(axis_nodes, links, fsTop.nodes_pic, PIC_PATH));
}
