...
from plugins.common.vt_manager_client import VMMClient
...
def create_vm_for_controller(island_obj, slice_obj, image_name):
    ...
    vm.name = image_name
    vm.image = Image.objects.get(name=image_name)
    vm.flavor = Flavor.objects.get(id=default_flavor_id)
    servers = [switch.virtualswitch.server for switch in slice_obj.get_virtual_switches_server()]
    vm.server = VMMClient().schedul(vm.flavor.cpu, vm.flavor.ram, vm.flavor.hdd, servers)
    vm.save()
